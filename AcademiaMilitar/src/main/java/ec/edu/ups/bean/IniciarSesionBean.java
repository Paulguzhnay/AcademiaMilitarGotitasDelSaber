	...
	public String iniciarSesion() {
		...
		for (int i=0; i<this.lista.size();i++) {
			if(iniciarSesion.getUsuario().equals(this.lista.get(i).getUsuario())){
				String[] correo = iniciarSesion.getUsuario().split("@");
				if(correo[1].equalsIgnoreCase("admin.com")) {
					...
					if(iniciarSesion.getContrasenia().equals(this.lista.get(i).getContrasenia())){
						...
						//FacesContext.getCurrentInstance().addMessage(null,new FacesMessage(FacesMessage.SEVERITY_WARN,"Ingreso correctamente loa datos", "no seas menso x2"));
